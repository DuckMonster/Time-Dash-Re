...
public class SYFlyerBig : SYCreep
{
	...
	Timer shootTimer = new Timer(2.1f, false);
	Timer chargeTimer = new Timer(1.2f, true);
	...
	public SYFlyerBig(Vector2 position, SYCreepCamp camp, Map map)
		: base(position, camp, map)
	{
		stats.MaxVelocity = 1.2f;
		stats.AccelerationTime = 4f;
		stats.DecelerationTime = 4f;

		chasingStats = new Stats();

		chasingStats.MaxVelocity = 2.8f;
	...
	void Shoot()
	{
		...
		if (rng.NextDouble() >= 0.5)
		{
			...
			for (int i = -1; i <= 1; i++)
			{
				Vector2 targetVector = Position + TKMath.GetAngleVector(aimDir + 25f * i);
		...
		} else
		{
			for(int i=0; i<9; i++)
			{
				Vector2 targetVector = Position + TKMath.GetAngleVector(45f * i);
		...
		shootTimer.Reset(1.5f + (float)rng.NextDouble() * 0.8f);
	}
